...
struct ManticoreMemoryInit : public Pass {
	...
	void execute(std::vector<std::string> args, Design *design) override
	{
		...
		for (const auto &m : top_mod->memories) {
		...
		for (const auto &cell : top_mod->cells()) {
		...
		for (const auto &p : mem_inits) {
			...
			std::string file_path = stringf("%s/%s.hex", prefix_path.c_str(), mid.substr(1).c_str());
			...
			top_mod->memories.at(mid)->set_string_attribute(ID::MEM_INIT_FILE, file_path);
		}
	}
